...
export function weaponAttackFlame(
  ...
  const pct = (Math.floor(itemLevel / 40) + 1) * tier * Math.pow(1.1, tier - 3);
...
const JOB_MAIN_STAT: Record<
  string,
  "str" | "dex" | "int" | "luk" | "hp" | "xenon"
> = {
  // 전사 (STR)
  히어로: "str",
  팔라딘: "str",
...
/** 직업명 정규화 후 주스탯 조회 (부분 일치로 신규 표기 변형도 흡수) */
function resolveJobMainStat(
  jobName: string,
): "str" | "dex" | "int" | "luk" | "hp" | "xenon" | undefined {
  const trimmed = jobName.trim();
  if (!trimmed) return undefined;
  const exact = JOB_MAIN_STAT[trimmed];
  if (exact) return exact;
  // 짧은 키(렌 등)는 오탐을 막기 위해 길이 2 이하는 포함 매칭하지 않는다
  let best: { name: string; stat: (typeof JOB_MAIN_STAT)[string] } | null =
    null;
  for (const [name, stat] of Object.entries(JOB_MAIN_STAT)) {
    if (name.length <= 2) continue;
    if (!trimmed.includes(name)) continue;
    if (!best || name.length > best.name.length) best = { name, stat };
  }
  return best?.stat;
}

/** 부스탯 짝 */
const SUB_STAT: Record<
  "str" | "dex" | "int" | "luk",
  "str" | "dex" | "int" | "luk"
> = {
  str: "dex",
...
export function jobProfile(
  ...
  const mapped = jobName ? resolveJobMainStat(jobName) : undefined;
...
export function effectScore(
  ...
  if (profile.useHp) {
    ...
    const att =
      profile.attackType === "magic" ? (e.magic ?? 0) : (e.attack ?? 0);
    score += att * w.attack;
