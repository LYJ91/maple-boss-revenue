...
export function SummaryBar({ summary }: { summary: AccountSummary }) {
  const capPct = Math.min(
    100,
    (summary.weeklyCrystalCount / RULES.worldWeeklySellLimit) * 100,
  );
  const overCap = summary.weeklyCrystalTotal > RULES.worldWeeklySellLimit;
    ...
    <section className="summary">
      ...
      <div className="stat-card">
        ...
        <strong className="stat-value">
          ...
          <span className="unit"> / {RULES.worldWeeklySellLimit}개</span>
        </strong>
        ...
        {overCap ? (
          <span className="stat-warn">
            결정 {summary.weeklyCrystalTotal}개 생산 — 가격 높은 순{' '}
            {RULES.worldWeeklySellLimit}개만 판매 가능
          </span>
        ) : (
          <span className="stat-sub">월드당 주 {RULES.worldWeeklySellLimit}개 판매 제한</span>
        )}
      </div>
    </section>
  );
