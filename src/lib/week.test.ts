...
describe("week helpers", () => {
  it("주차의 마지막 날(수요일)과 이전 주차를 계산한다", () => {
    expect(weekEndDate("2026-07-09")).toBe("2026-07-15");
    expect(shiftWeek("2026-07-16", -1)).toBe("2026-07-09");
  });

  it("넥슨 과거 조회 창(1~13일 전)만 허용한다", () => {
    const now = new Date(2026, 6, 16, 12, 0, 0); // 2026-07-16
    expect(canQueryNexonDate("2026-07-15", now)).toBe(true);
